...
# Run the application
dotnet run --project src/MinimalTips.Website.Web

# Or run with Aspire (if using)
dotnet run --project src/MinimalTips.Website.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/MinimalTips.Website.Web -s src/MinimalTips.Website.Web
dotnet run --project src/MinimalTips.Website.Web
```
